...
def self_restart():
    """Generate .restart.sh and execute it to fully restart ClawX.

    This replaces the old SIGUSR1 approach. The script:
      1. Waits 5s (gives caller time to return)
      2. Kills the ClawX process tree
      3. Waits for full exit (up to 15s)
      4. Re-launches via uv run clawx.py in the same terminal

    Because resume_last is typically true, Claude Code will --continue
    the same session thread.
    """
    ...
    script = f"""#!/bin/bash
# Auto-generated by ClawX restart — do not edit
echo "[$(date)] Restart initiated, killing PID {pid}..." >> {project_dir}/logs/restart.log
sleep 5

# Kill ClawX process
kill {pid} 2>/dev/null

# Wait for old process to fully exit (up to 15s)
...
sleep 2
echo "[$(date)] Old process gone, starting new ClawX" >> {project_dir}/logs/restart.log
cd {project_dir}
exec uv run clawx.py
"""
    restart_sh.write_text(script)
